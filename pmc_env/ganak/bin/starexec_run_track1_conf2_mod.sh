...
./ganak/bin/doalarm ${tout_be} ./ganak/bin/arjun --recomp 1 --backbone 1 $cleanfile --elimtofile $preprocessed_cnf_file | sed "s/^/c o /"
...
./ganak/bin/sharpSAT-td -decot 120 -decow 100 -tmpdir . -cs ${cache_size} -pptoutdiv 10 -ppstr "P" $cleanfile | tee $solfile | sed "s/^/c o /"
